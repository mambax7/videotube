...
function videotube_latest_play($options) {
  ...
  $videosize = $options[0];
  $result = $xoopsDB->queryF("SELECT id, uid, date, code, title, artist, views, service, embedcode, fullembedcode FROM ".$xoopsDB->prefix()."_vp_videos WHERE pub='1' ORDER BY date DESC");
  ...
  if ($vp_vid_disp['service'] == 10) {
    ...
    $block['un_lt_videotitle'] = $vp_vid_disp['title'];
    $block['un_lt_localviews'] = $vp_vid_disp['views'];
    $block['un_lt_submitter'] = $vp_vid_disp['submitter'];
    $block['un_lt_vsubmitdate'] = $vp_vid_disp['date'];
    ...
    foreach ($xml->channel->item as $item) {
      ...
      $bt_lt_videotitle = $item->title;
      $bt_lt_published = $item->pubDate;
      $bt_lt_videoauthor = $iblip->user;
      $bt_lt_duration = $iblip->runtime;

	  $block['bt_lt_videotitle'] = $bt_lt_videotitle;
	  $block['bt_lt_videoauthor'] = $bt_lt_videoauthor;
	  $block['bt_lt_duration'] = $bt_lt_duration;
	  $block['bt_lt_published'] = $bt_lt_published;
	  $block['bt_lt_localviews'] = $vp_vid_disp['views'];
	  $block['bt_lt_submitter'] = $vp_vid_disp['submitter'];
	  $block['bt_lt_vsubmitdate'] = $vp_vid_disp['date'];
	  $block['bt_lt_vsubmittime'] = $vp_vid_disp['time'];
    ...
    foreach ($xml->channel->item as $item) {
      ...
      $mc_lt_videotitle = $item->title;
      $mc_lt_published = $item->pubDate;
      $mc_lt_videoauthor = $item->author;
      
      $contentattrs = $imedia->content[0]->attributes();
      $mc_lt_duration = addslashes($contentattrs['duration']);
      
      $description = $item->description;
      $parsedesc = explode('|',$description);
      $parsedesc2 = explode(' ',$parsedesc[1]);
      $mc_lt_videoviews = $parsedesc2[1];

	  $block['mc_lt_videotitle'] = $mc_lt_videotitle;
	  $block['mc_lt_videoauthor'] = $mc_lt_videoauthor;
	  $block['mc_lt_videoviews'] = $mc_lt_videoviews;
	  $block['mc_lt_duration'] = $mc_lt_duration;
	  $block['mc_lt_published'] = $mc_lt_published;
	  $block['mc_lt_localviews'] = $vp_vid_disp['views'];
	  $block['mc_lt_submitter'] = $vp_vid_disp['submitter'];
	  $block['mc_lt_vsubmitdate'] = $vp_vid_disp['date'];
	  $block['mc_lt_vsubmittime'] = $vp_vid_disp['time'];
    }
  }elseif ($vp_vid_disp['service'] == 2) {
    ...
    foreach ($xml->channel->item as $item) {
      ...
      $dm_lt_videotitle = $item->title;
      $dm_lt_published = $item->pubDate;
      $dm_lt_videoauthor = $iitunes->author;
      $dm_lt_videoviews = $idm->views;

      $contentattrs = $imedia->group->content[0]->attributes();
      $dm_lt_duration = $contentattrs['duration'];
	
	  $block['dm_lt_videotitle'] = $dm_lt_videotitle;
	  $block['dm_lt_videoauthor'] = $dm_lt_videoauthor;
	  $block['dm_lt_videoviews'] = $dm_lt_videoviews;
	  $block['dm_lt_duration'] = $dm_lt_duration;
	  $block['dm_lt_published'] = $dm_lt_published;
	  $block['dm_lt_localviews'] = $vp_vid_disp['views'];
	  $block['dm_lt_submitter'] = $vp_vid_disp['submitter'];
	  $block['dm_lt_vsubmitdate'] = $vp_vid_disp['date'];
	  $block['dm_lt_vsubmittime'] = $vp_vid_disp['time'];
    }
  }
 return $block;
}

function latest_edit_play($options)
{
	$form  = "<table border='0'>";
