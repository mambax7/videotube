...
//  Date:       01/06/2009                                                   //
//  Module:     Video Tube                                                   //
//  File:       blocks/vp_random_wall.php                                    //
//  Version:    1.81                                                         //
//  ------------------------------------------------------------------------ //
//  Change Log                                                               //
//  ***                                                                      //
...
function videotube_random_wall($options) {
		...
		$result = $xoopsDB->queryF("SELECT id, uid, date, code, title, artist, views, service, thumb FROM ".$xoopsDB->prefix()."_vp_videos WHERE pub='1' ORDER BY rand() limit 0, $numVideos");
		...
		while($vp_vid = $xoopsDB->fetcharray($result)) {
			...
			$vp_vid_disp['title'] = addslashes($vp_vid['title']);
			$vp_vid_disp['artist'] = addslashes($vp_vid['artist']);
...
function random_edit_wall($options)
{
	$form  = "<table border='0'>";
	$form .= "<tr><td>"._BL_VP_RD_VIDEOWALL."</td><td>";
