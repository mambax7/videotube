...
//  Date:       01/06/2009                                                   //
//  Module:     Video Tube                                                   //
//  File:       blocks/vp_random_play.php                                    //
//  Version:    1.81                                                         //
//  ------------------------------------------------------------------------ //
//  Change Log                                                               //
//  ***                                                                      //
...
function videotube_random_play($options) {
  ...
  $videosize = $options[0];
  $result = $xoopsDB->queryF("SELECT id, uid, date, code, title, artist, views, service, embedcode, fullembedcode FROM ".$xoopsDB->prefix()."_vp_videos WHERE pub='1' ORDER BY RAND()");
  ...
  if ($vp_vid_disp['service'] == 10) {
    ...
    $block['un_rd_videotitle'] = $vp_vid_disp['title'];
    $block['un_rd_localviews'] = $vp_vid_disp['views'];
    $block['un_rd_submitter'] = $vp_vid_disp['submitter'];
    $block['un_rd_vsubmitdate'] = $vp_vid_disp['date'];
    ...
    foreach ($xml->channel->item as $item) {
      ...
      $bt_rd_videotitle = $item->title;
      $bt_rd_published = $item->pubDate;
      $bt_rd_videoauthor = $iblip->user;
      $bt_rd_duration = $iblip->runtime;

	  $block['bt_rd_videotitle'] = $bt_rd_videotitle;
	  $block['bt_rd_videoauthor'] = $bt_rd_videoauthor;
	  $block['bt_rd_duration'] = $bt_rd_duration;
	  $block['bt_rd_published'] = $bt_rd_published;
	  $block['bt_rd_localviews'] = $vp_vid_disp['views'];
	  $block['bt_rd_submitter'] = $vp_vid_disp['submitter'];
	  $block['bt_rd_vsubmitdate'] = $vp_vid_disp['date'];
	  $block['bt_rd_vsubmittime'] = $vp_vid_disp['time'];
    ...
    foreach ($xml->channel->item as $item) {
      ...
      $mc_rd_videotitle = $item->title;
      $mc_rd_published = $item->pubDate;
      $mc_rd_videoauthor = $item->author;
      
      $contentattrs = $imedia->content[0]->attributes();
      $mc_rd_duration = addslashes($contentattrs['duration']);
      
      $description = $item->description;
      $parsedesc = explode('|',$description);
      $parsedesc2 = explode(' ',$parsedesc[1]);
      $mc_rd_videoviews = $parsedesc2[1];

	  $block['mc_rd_videotitle'] = $mc_rd_videotitle;
	  $block['mc_rd_videoauthor'] = $mc_rd_videoauthor;
	  $block['mc_rd_videoviews'] = $mc_rd_videoviews;
	  $block['mc_rd_duration'] = $mc_rd_duration;
	  $block['mc_rd_published'] = $mc_rd_published;
	  $block['mc_rd_localviews'] = $vp_vid_disp['views'];
	  $block['mc_rd_submitter'] = $vp_vid_disp['submitter'];
	  $block['mc_rd_vsubmitdate'] = $vp_vid_disp['date'];
	  $block['mc_rd_vsubmittime'] = $vp_vid_disp['time'];
    }
  }elseif ($vp_vid_disp['service'] == 2) {
    ...
    foreach ($xml->channel->item as $item) {
      ...
      $dm_rd_videotitle = $item->title;
      $dm_rd_published = $item->pubDate;
      $dm_rd_videoauthor = $iitunes->author;
      $dm_rd_videoviews = $idm->views;

      $contentattrs = $imedia->group->content[0]->attributes();
      $dm_rd_duration = $contentattrs['duration'];
	
	  $block['dm_rd_videotitle'] = $dm_rd_videotitle;
	  $block['dm_rd_videoauthor'] = $dm_rd_videoauthor;
	  $block['dm_rd_videoviews'] = $dm_rd_videoviews;
	  $block['dm_rd_duration'] = $dm_rd_duration;
	  $block['dm_rd_published'] = $dm_rd_published;
	  $block['dm_rd_localviews'] = $vp_vid_disp['views'];
	  $block['dm_rd_submitter'] = $vp_vid_disp['submitter'];
	  $block['dm_rd_vsubmitdate'] = $vp_vid_disp['date'];
	  $block['dm_rd_vsubmittime'] = $vp_vid_disp['time'];
    }
  }
 return $block;
}

function random_edit_play($options)
{
	$form  = "<table border='0'>";
