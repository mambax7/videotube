...
function videotube_latest_h($options) {
		...
		$result = $xoopsDB->queryF("SELECT id, uid, date, code, title, artist, views, thumb, service, servicename FROM ".$xoopsDB->prefix()."_vp_videos WHERE pub='1' ORDER BY date DESC limit 0, $numVideos");
...
function latest_edit_h($options)
{
	$form  = "<table border='0'>";
	$form .= "<tr><td>"._BL_VP_LT_NUMVIDEOS."</td><td>";
